...
import { getProductImageBackground, isPolyvoreProduct } from "../../utils/imageUrl";
...
const WardrobePiecesPanel = ({
  ...
  >
    ...
    <Box
      style={{
        display: "grid",
        gridTemplateColumns: items.length > 0 ? "repeat(3, 1fr)" : "1fr",
        gap: 2,
    ...
    >
      ...
      items.map((item, index) => {
        ...
        <Text ta="center" c="rgba(0,0,0,0.45)" px="md" py="xl" style={{ fontSize: 11, letterSpacing: 0.5 }}>
          {isAuthenticated
            ? "Henüz dolabınızda parça yok. Mağazadan alışveriş yaparak başlayın."
            : "Dolabınızı görmek için giriş yapın veya alışveriş yapın."}
        </Text>
      )}
    </Box>
...
const MyWardrobe = () => {
  ...
  const [displayItems, setDisplayItems] = useState<WardrobeItem[]>([]);
